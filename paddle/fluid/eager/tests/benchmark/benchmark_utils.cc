...
void benchmark_eager_matmul(const EagerTensor& X, const EagerTensor& Y,
  ...
  for (size_t i = 0; i < max_num_runs; i++) {
    input_tensor0 = egr::matmul(input_tensor0, Y, false,
                                false, true);
  }
...
void benchmark_eager_mlp(const EagerTensor& X,
  ...
  for (size_t i = 0; i < MLP_NUM_LINEAR; i++) {
    EagerTensor Out0 = egr::matmul(input0, Ws[i], false,
                                   false, true);

    input0 =
        egr::elementwise_add(Out0, Bs[i], -1, true);
  }

  EagerTensor Out = egr::reduce_sum(input0, {0}, false,
                                    true, -1,
                                    -1, true);
