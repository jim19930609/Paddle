...
TEST(Benchmark, FluidMLPCUDA) {
  ...
  for (const std::string& mode : {"Accuracy", "WarmUp", "Performance"}) {
    ...
    paddle::memory::Copy(place, mutable_b1, platform::CPUPlace(),
                         b1_src_data.data(), sizeof(float) * b1_src_data.size(),
                         stream);
    ...
    paddle::memory::Copy(place, mutable_b2, platform::CPUPlace(),
                         b2_src_data.data(), sizeof(float) * b2_src_data.size(),
                         stream);
