...
egr::EagerTensor matmul(const egr::EagerTensor& x, const egr::EagerTensor& y,
  ...
  if (require_any_grad) {
    PassStopGradient(false , p_autograd_out);

    // Init GradNode
    auto matmul_node =
        std::make_shared<GradNodeMatmul>(1, 2);
    ...
    // Set Next Edges
    matmul_node->AddEdges(*p_autograd_x, 0);
    matmul_node->AddEdges(*p_autograd_y, 1);
    ...
    // Set Grad out rank as same as fwd input and set stop gradient to bwd
    matmul_node->SetGradOutMeta(*p_autograd_x, 0);
    matmul_node->SetGradOutMeta(*p_autograd_y, 1);
    // Set Grad out rank as same as fwd input and set stop gradient to bwd
    matmul_node->SetGradInMeta(*p_autograd_out, 0);
    p_autograd_out->SetSingleOutRankWithSlot(0, 0);
